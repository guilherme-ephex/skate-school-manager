...
export const Calendar: React.FC = () => {
    ...
    useEffect(() => {
        if (user) {
            setFilterMode(user.role === 'ADMIN' ? 'all' : 'mine');
        }
    }, [user]);
    ...
    const getClassesForDate = (date: Date): ClassSchedule[] => {
        ...
        // Filter based on mode and user
        const filteredClasses = classes.filter(cls => {
            // Apply teacher filter if in "mine" mode
            if (filterMode === 'mine' && user && user.role === 'TEACHER') {
                if (cls.teacher_id !== user.id) return false;
            }
        ...
        return filteredClasses.map(cls => ({ class: cls, dayOfWeek: dayName }));
    };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 h-[calc(100vh-100px)]">
            <div className="xl:col-span-2 flex flex-col h-full">
                <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6 gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-text-light dark:text-text-dark">Calendário de Aulas</h1>
                        <p className="text-muted dark:text-muted-dark">Selecione um dia para ver os detalhes.</p>
                    </div>
                ...
                <div className="bg-card-light border border-border-light rounded-xl p-4 flex-grow flex flex-col shadow-sm">
                                        ...
                                        <div className="flex gap-1 mt-1 flex-wrap">
                                            {dayClasses.map(({ class: cls }) => {
                                                ...
                                                return (
                                                    <div
                                                        key={key}
                                                        className={`w-2 h-2 rounded-full ${status.status === 'cancelled' ? 'bg-red-500' : 'bg-green-500'
                                                            }`}
                    ...
                    <div className="mt-4 flex flex-wrap gap-4 text-xs text-muted dark:text-muted-dark">
                        ...
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-red-500"></div>Cancelada
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="xl:col-span-1">
                <div className="bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark rounded-xl p-6 h-full shadow-sm flex flex-col">
                    <h2 className="text-2xl font-bold mb-4 text-text-light dark:text-text-dark">{formatDate(selectedDate)}</h2>

                    {loading ? (
                        <div className="flex items-center justify-center h-full">
                            <p className="text-muted dark:text-muted-dark">Carregando...</p>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <p className="text-muted dark:text-muted-dark">Nenhuma aula agendada para este dia.</p>
                        </div>
                    ) : (
                        <>
                            <h3 className="font-bold text-lg mb-4 text-text-light dark:text-text-dark">Aulas do dia</h3>
                            <div className="flex flex-col gap-4 overflow-y-auto pr-2">
                                {selectedDateClasses.map(({ class: cls }) => {
                                    ...
                                    return (
                                        <div
                                            key={cls.id}
                                            className={`p-4 rounded-lg bg-gray-50 dark:bg-gray-800 border-l-4 ${cls.level === 'Iniciante' ? 'border-blue-400' :
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-3">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-muted dark:text-muted-dark">{cls.time}</p>
                                                        ...
                                                        <div className="flex items-center gap-2 mt-2">
                                                            {cls.teacher.avatar_url ? (
                                                                ...
                                                                />
                                                                    ...
                                                                    <span className="material-symbols-outlined text-xs text-gray-500 dark:text-gray-400">person</span>
                                                                </div>
                                                            )}
                                                            <span className="text-xs text-muted dark:text-muted-dark">{cls.teacher.full_name}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            ...
                                            <Link
                                                to={`/attendance?classId=${cls.id}&date=${dateStr}`}
                                                className={`w-full block text-center py-2 rounded-lg text-sm font-bold transition-colors ${status
                                                    ? 'bg-white dark:bg-gray-700 border border-primary dark:border-primary-dark text-primary dark:text-primary-dark hover:bg-primary/5 dark:hover:bg-primary-dark/10'
                                                    : 'bg-primary dark:bg-primary-dark text-white hover:bg-primary/90 dark:hover:bg-primary-dark/90'
                                                    }`}
                                            >
